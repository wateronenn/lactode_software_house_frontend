...
export default function RoomDetailPage() {
  ...
  if (loadError || !hotel || !room) {
    return (
      <main className="min-h-screen mx-auto max-w-7xl px-8 py-8 lg:px-16">
        <div>
          <Button variant="disabled" className="btn-md" onClick={() => router.push('/admin/hotels')}>
            Back
          </Button>
        </div>
  ...
  const roomEditHref =
    hotelId && roomId ? `/admin/hotels/${hotelId}/rooms/${roomId}/edit` : '/admin/hotels';
  const backToHotel = hotelId ? `/admin/hotels/${hotelId}` : '/admin/hotels';
