...
export default function AdminDashboardPage() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      const [bookingsResponse, hotelsResponse, usersResponse] = await Promise.all([
      ...
      for (const targetUser of users) {
        nextUserById[targetUser._id] = targetUser;
      }
      ...
      setSummary({
        totalBookings: readBackendCount(bookingsResponse),
        totalHotels: readBackendCount(hotelsResponse),
        totalUsers: readBackendCount(usersResponse),
      });
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6 lg:px-8">
        <div className="rounded-[28px] border border-amber-200 bg-amber-50 p-10 text-amber-700 shadow-soft">
          Admin access only. Sign in with an admin account to view this dashboard.
        </div>
      </main>
    );
  }
